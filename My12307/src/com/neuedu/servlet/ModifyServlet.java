...
public class ModifyServlet extends HttpServlet {
	 ...
	 * @throws Exception
	 */
	private void checkLoginName(HttpServletRequest request, 
	...
	private void doModify(HttpServletRequest request, 
		...
		if(isMultipart) {
			...
			for(FileItem item:fileItems) {
				if(item.isFormField()) {
					...
					if(null!=fileName&&fileName.indexOf(".")!=-1) {
						//生成唯一文件名
						String newName=new Date().getTime()+"."+fileName.substring(fileName.lastIndexOf(".")+1);
						//定义一个文件对象，用来保存上传的文件内容
						File fullFile=new File(getServletContext().getRealPath("/images/photo/"),newName);
						//把item中的文件数据，写入到服务器指定文件中
						item.write(fullFile);
						imagePath=newName;
					}
					//删除临时文件
			...
			us.login(loginName,((User)se.getAttribute(Constants.userKey)).getPassword());
